...
const uint8_t HomeAutomation::debugLevel( DEBUG_STATE_L3 | DEBUG_LEVEL_LOW );

Timestamp HomeAutomation::lastMemoryReportTime;

uint8_t HomeAutomation::reportMemoryStatusTime( 0 );
...
void HomeAutomation::run()
{
   ...
   if ( inStartUp() )
   {
      ...
      reportMemoryStatusTime = conf.getReportMemoryStatusTime();
      SystemTime::ticksPerSecondAdjustment = conf.getTimeCorrectionValue();
      DEBUG_H2( FSTR( "deviceId: 0x" ), HACF::deviceId );
...
{
   DEBUG_H4( FSTR( ".readMemory(" ), parameter.address, ',', parameter.length );
   DEBUG_L1( ')' );
