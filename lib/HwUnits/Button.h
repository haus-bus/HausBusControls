...
class Button : public Reactive
{
   ...
   public:
      ...
      PortPinUnit* getFeedbackLed() const
      {
         return feedbackLed;
      }

      void setFeedbackLed( PortPinUnit* p_Led )
      {
         feedbackLed = p_Led;
      }
...
{
   controlFrame.setDataLength( sizeof( getResponse() ) + sizeof( getParameter().state ) );
   setResponse( STATUS );
